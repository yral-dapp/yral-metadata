...
async fn set_user_metadata(state: State<AppState>, path: Path<Principal>, metadata: Json<UserMetadata>) -> Result<Json<ApiResult<SetUserMetadataRes>>> {
    let user = path.to_text();

    let req = state.kv_namespace.write_kv(user)
        .metadata(&metadata.0)?;
...
async fn get_user_metadata(state: State<AppState>, path: Path<Principal>) -> Result<Json<ApiResult<UserMetadata>>> {
    let user = path.to_text();
